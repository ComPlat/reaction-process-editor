...
import { Button, Card, Form, FormGroup, Input, Label, Modal, ModalHeader, ModalBody } from 'reactstrap';
...
import OntologyEditableItemList from './OntologyEditableItemList';
import OntologyEditableTextList from './OntologyEditableTextList';
...
const OntologyForm = ({ ontology, isOpen, onFinished }) => {
  ...
  const renderDeviceFields = () => {
    return (
      <div className="row">
        <FormGroup className="col-4 border"  >
          <OntologyEditableItemList label={'Detectors'} items={currentOntology.detectors} onChange={handleChange('detectors')} filteredOntologies={filteredOntologies} >
          </OntologyEditableItemList>

        </FormGroup>
        <FormGroup className="col-4 border"  >
          <OntologyEditableItemList label={'Solvents'} items={currentOntology.solvents} onChange={handleChange('solvents')} filteredOntologies={filteredOntologies} >
          </OntologyEditableItemList>
        </FormGroup>
        <FormGroup className="col-4 border"  >
          <OntologyEditableTextList label={'Stationary Phase'} items={currentOntology.stationary_phase} onChange={handleChange('stationary_phase')}>
          </OntologyEditableTextList>
        </FormGroup>
      </div>
    )
  }
  const renderRolesDependenciesForm = () => {
    return (
      <>
        ...
        <FormGroup floating className="mt-3 col-3">
          <Select
            placeholder={''}
      ...
      >
        ...
        <Form className='m-3'>
          <div className='row'>
            ...
            <FormGroup floating className="col-10">
              <Input
                label={"Ontology-id"}
                type="textfield"
          ...
          <div className='row'>
            <FormGroup floating className="col-6">
              <Input
                type="textfield"
          ...
          <div className='row mt-3'>
            ...
            <div className='col-2'>
              Selectable: {filteredOntologies.length} / {ontologies.length}
            </div>
          </div>
          <FormButtons
  ...
  return (
    <>
      ...
      <Modal
        ...
        backdrop={"static"}
      >
        <ModalHeader>  {!!ontology.ontology_id ? "Edit Ontology – " + OntologiesInfoDecorator.infoLabelForId({ ontologyId: ontology.ontology_id, ontologies: ontologies }) : 'Create Ontology'}</ModalHeader>
        <ModalBody>
          {renderForm(ontology)}
        </ModalBody>
      </Modal>
